HA$PBExportHeader$u_genidwdm_splitbar_horizontal.sru
$PBExportComments$Horizontal splitbar object
forward
global type u_genidwdm_splitbar_horizontal from statictext
end type
end forward
...
global u_genidwdm_splitbar_horizontal u_genidwdm_splitbar_horizontal
...
public function boolean of_forget_topcontrol (dragobject adrg_ctrl)
public function boolean of_forget_bottomcontrol (dragobject adrg_ctrl)
public function boolean of_forget_control (dragobject adrg_ctrl)
public subroutine updateindexes ()
public function integer gettitleheight ()
end prototypes
...
If KeyDown(keyLeftButton!) and il_prev_pos <> ypos Then
	// min bound for range of movement
	...
	If ipo_parent.TypeOf() = Window! Then
		...
		li_maxrange = iw_parent.height - ii_maxrange - GetTitleHeight() - height
	Else
		ldrg_parent = ipo_parent
		li_pointer = ldrg_parent.PointerY()
		li_maxrange = ldrg_parent.height - ii_maxrange - height
	End If
...
FOR li_cnt = 1 TO li_max
	if not isvalid(idrg_top[li_cnt]) or isnull(idrg_top[li_cnt]) then continue
	...
	if isvalid( ipo_resizer ) then
		lnv_resizer = ipo_resizer.of_findobject( idrg_top[li_cnt] )
...
FOR li_cnt = 1 TO li_max
	if not isvalid(idrg_bottom[li_cnt]) or isnull(idrg_bottom[li_cnt]) then continue
	...
	if isvalid( ipo_resizer ) then
		lnv_resizer = ipo_resizer.of_findobject( idrg_bottom[li_cnt] )
		if isvalid( lnv_resizer ) then
...
il_offsets_top[li_max] = this.y - adrg_top.y - adrg_top.height
...
li_max = upperbound(idrg_bottom[])
for i = 1 to li_max
	if idrg_bottom[i] = adrg_ctrl then
		setnull(idrg_bottom[i])
		updateindexes( )
		return true
	end if
next
...
public subroutine updateindexes ();
int i
for i = 1 to upperbound(idrg_top[])
	if not isnull(idrg_top[i]) then
		ii_first_top = i
		exit
	end if
next
for i = 1 to upperbound(idrg_bottom[])
	if not isnull(idrg_bottom[i]) then
		ii_first_bottom = i
		exit
	end if
next
...
public function integer gettitleheight ();// returns the window standard title bar height (in PB units)
...
on u_genidwdm_splitbar_horizontal.create
end on

on u_genidwdm_splitbar_horizontal.destroy
end on
